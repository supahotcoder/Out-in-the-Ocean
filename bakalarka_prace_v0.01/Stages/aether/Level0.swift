...
class Level0: GameScene {
    ...
    override func didMove(to view: SKView) {
        ...
        if joystick.isDynamic{
            self.updateStoryText(with: "By pressing anywhere on display a joystick will be shown", around: self.playerNode!, fadeOut: 1,forDuration: 1.5)
        }else{
            self.updateStoryText(with: "On your left a joystick will be shown", around: self.playerNode!, fadeOut: 1,forDuration: 1)
        }
        self.updateStoryText(with: "Use it for movement", around: self.playerNode!, fadeOut: 1, forDuration: 0.8)
        self.updateStoryText(with: "Of course the joystick is working,\nyou just need to wait until I finish this introduction.", around: self.playerNode!, fadeOut: 1,forDuration: 1)
        ...
        self.updateGoalText(with: "Can't find any?", around: self.playerNode!, displayIn: 50)
        self.updateGoalText(with: "Don't worry there weren't any", around: self.playerNode!, displayIn: 55)
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if -(startDate?.timeIntervalSinceNow ?? 0) > 75 + totalTimeInMenu, !isChangingLevel{
